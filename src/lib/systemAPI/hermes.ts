  ...
  async ensureWhatsAppBridgeDeps(
    ...
    return runHermesShell(
      [
        ...
        '}',
        '',
        '# ── Sentinel-based health check (matches upstream Hermes PR #13499) ──',
        '# A partial npm install (e.g. user pressed Ctrl+C, network blip) leaves',
        '# node_modules/ present but with required deps missing or only',
        '# package.json on disk without lib/index.js. Treat any of those as',
        '# "needs install" so we never fall through to a broken bridge.',
        'WHY=""',
        'NEEDS_INSTALL=0',
        'BAILEYS_PKG="node_modules/@whiskeysockets/baileys/package.json"',
        'add_reason() { WHY="$WHY\n  - $1"; }',
        'if [ ! -d node_modules ]; then',
        '  NEEDS_INSTALL=1; add_reason "node_modules/ missing"',
        'fi',
        'for sentinel in "$BAILEYS_PKG" \\',
        '                "node_modules/express/package.json" \\',
        '                "node_modules/qrcode-terminal/package.json" \\',
        '                "node_modules/pino/package.json"; do',
        '  if [ ! -f "$sentinel" ]; then',
        '    NEEDS_INSTALL=1; add_reason "missing $sentinel"',
        '  fi',
        'done',
        'if [ -f "$BAILEYS_PKG" ]; then',
        '  # Verify Baileys main entry actually exists. Reading "main" via node',
        '  # avoids hard-coding lib/index.js in case upstream renames it.',
        '  BAILEYS_MAIN="$("$NODE_BIN" -e "try{const p=require(\'./node_modules/@whiskeysockets/baileys/package.json\');process.stdout.write(p.main||\'lib/index.js\')}catch(e){process.stdout.write(\'lib/index.js\')}" 2>/dev/null)"',
        '  if [ -n "$BAILEYS_MAIN" ] && [ ! -f "node_modules/@whiskeysockets/baileys/$BAILEYS_MAIN" ]; then',
        '    NEEDS_INSTALL=1; add_reason "Baileys $BAILEYS_MAIN missing — partial install"',
        '  fi',
        'fi',
        '# Stale lock heuristic: package-lock.json newer than node_modules marker',
        '# means the lock was updated by `git pull` but never installed. Reinstall.',
        'if [ "$NEEDS_INSTALL" = "0" ] && [ -f package-lock.json ] && [ -f node_modules/.package-lock.json ]; then',
        '  if [ package-lock.json -nt node_modules/.package-lock.json ]; then',
        '    NEEDS_INSTALL=1; add_reason "package-lock.json newer than node_modules/.package-lock.json"',
        '  fi',
        'fi',
        '# ESM load probe: Baileys v7 is ESM, so require() would lie. import()',
        '# matches what bridge.js itself does at runtime.',
        ...
        '    NEEDS_INSTALL=1; add_reason "import(@whiskeysockets/baileys) failed: $(tr \"\\n\" \" \" </tmp/ronbot-baileys-load-err | cut -c1-160)"',
        '  fi',
        'fi',
        '',
        'if [ "$NEEDS_INSTALL" = "1" ]; then',
        '  echo "[ronbot] WhatsApp bridge install is incomplete — repairing."',
        '  printf "[ronbot] reasons:%b\\n" "$WHY"',
        '  # Stop any bridge process holding files inside node_modules so rm -rf',
        '  # actually succeeds on Linux. Safe no-op if nothing is running.',
        '  pkill -f "whatsapp-bridge/bridge.js" >/dev/null 2>&1 || true',
        '  sleep 1',
        '  pkill -9 -f "whatsapp-bridge/bridge.js" >/dev/null 2>&1 || true',
        '  rm -rf node_modules package-lock.json 2>/dev/null || true',
        '  run_npm_install "installing WhatsApp bridge dependencies (npm install)…" ""',
        '  rc=$?',
        '  if [ $rc -ne 0 ]; then',
        '    echo "[ronbot] First npm install attempt failed (exit $rc). Verifying npm cache and retrying with --force…" >&2',
        ...
        '    fi',
        '  fi',
        '  # Re-verify sentinels after install. If any are still missing the',
        '  # registry returned an incomplete tarball; surface that as failure',
        ...
        '                  "node_modules/pino/package.json"; do',
        '    if [ ! -f "$sentinel" ]; then',
        '      echo "[ronbot] npm install completed but $sentinel is still missing." >&2',
        '      exit 1',
        '    fi',
        '  done',
        '  if ! "$NODE_BIN" --input-type=module -e "import(\'@whiskeysockets/baileys\').then(()=>process.exit(0)).catch(e=>{process.stderr.write(String(e&&e.message||e));process.exit(1)})" 2>/tmp/ronbot-baileys-load-err; then',
  ...
  async auditWhatsAppBridgeRuntime(): Promise<{
    ...
    const r = await runHermesShell(
      [
        ...
        'fi',
        // bridge deps existence (sentinel-based) + ESM load probe (Baileys v7 ships ESM).
        // Mirrors upstream Hermes PR #13499: a partial npm install can leave',
        // node_modules present but with required deps missing — treat that as FAIL.',
        'BAILEYS_PKG="$BAILEYS_DIR/package.json"',
        'MISSING_SENT=""',
        'for sentinel in "$BAILEYS_PKG" \\',
        '                "$BRIDGE_DIR/node_modules/express/package.json" \\',
        '                "$BRIDGE_DIR/node_modules/qrcode-terminal/package.json" \\',
        '                "$BRIDGE_DIR/node_modules/pino/package.json"; do',
        '  if [ ! -f "$sentinel" ]; then MISSING_SENT="$MISSING_SENT $sentinel"; fi',
        'done',
        'if [ -z "$MISSING_SENT" ] && [ -x "$NODE_BIN" ] && [ -f "$BAILEYS_PKG" ]; then',
        '  BAILEYS_MAIN="$("$NODE_BIN" -e "try{const p=require(\'$BAILEYS_PKG\');process.stdout.write(p.main||\'lib/index.js\')}catch(e){process.stdout.write(\'lib/index.js\')}" 2>/dev/null)"',
        '  if [ -n "$BAILEYS_MAIN" ] && [ ! -f "$BAILEYS_DIR/$BAILEYS_MAIN" ]; then',
        '    MISSING_SENT="$MISSING_SENT $BAILEYS_DIR/$BAILEYS_MAIN"',
        '  fi',
        'fi',
        'if [ -z "$MISSING_SENT" ]; then',
